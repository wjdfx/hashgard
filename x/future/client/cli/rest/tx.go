...
func registerTxRoutes(cliCtx context.CLIContext, r *mux.Router, cdc *codec.Codec) {
	...
	r.HandleFunc(fmt.Sprintf("/%s/deposit-to/{%s}/{%s}", types.Future, boxrest.ID, boxrest.Amount), boxrest.PostDepositHandlerFn(cdc, cliCtx, types.Future, types.Inject)).Methods("POST")
	r.HandleFunc(fmt.Sprintf("/%s/fetch/{%s}/{%s}", types.Future, boxrest.ID, boxrest.Amount), boxrest.PostDepositHandlerFn(cdc, cliCtx, types.Future, types.Cancel)).Methods("POST")
