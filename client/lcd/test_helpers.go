...
func InitializeTestLCD(
	...
	db := dbm.NewMemDB()
	app := happ.NewHashgardApp(logger, db, nil, true)
	cdc = happ.MakeCodec()
	...
	for i := 0; i < nValidators; i++ {
		...
		msg := staking.NewMsgCreateValidator(
			...
			pubKey,
			sdk.NewCoin(happ.StakeDenom, sdk.NewInt(int64(delegation))),
		...
		accAuth.Coins = sdk.Coins{sdk.NewInt64Coin(happ.StakeDenom, 150)}
		accs = append(accs, happ.NewGenesisAccount(&accAuth))
	}

	appGenState := happ.NewDefaultGenesisState()
	appGenState.Accounts = accs
	genDoc.AppState, err = cdc.MarshalJSON(appGenState)
	require.NoError(t, err)
	genesisState, err := happ.HashgardAppGenState(cdc, *genDoc, genTxs)
	require.NoError(t, err)

	// add some tokens to init accounts
	for _, addr := range initAddrs {
		accAuth := auth.NewBaseAccountWithAddress(addr)
		accAuth.Coins = sdk.Coins{sdk.NewInt64Coin(happ.StakeDenom, 100)}
...
func doTransferWithGas(t *testing.T, port, seed, name, memo, password string, addr sdk.AccAddress, gas string,
	...
	receiveInfo, _, err := kb.CreateMnemonic("receive_address", cryptoKeys.English, happ.DefaultKeyPass, cryptoKeys.SigningAlgo("secp256k1"))
	require.Nil(t, err)
...
func doDelegate(t *testing.T, port, name, password string,
	...
	msg := msgDelegationsInput{
		...
		Delegation:    sdk.NewInt64Coin(happ.StakeDenom, amount),
	}
...
func doSubmitProposal(t *testing.T, port, seed, name, password string, proposerAddr sdk.AccAddress, amount int64, fees sdk.Coins) (resultTx ctypes.ResultBroadcastTxCommit) {
	...
	pr := postProposalReq{
		...
		InitialDeposit: sdk.Coins{sdk.NewCoin(happ.StakeDenom, sdk.NewInt(amount))},
		BaseReq:        baseReq,
	}
...
func doDeposit(t *testing.T, port, seed, name, password string, proposerAddr sdk.AccAddress, proposalID uint64, amount int64, fees sdk.Coins) (resultTx ctypes.ResultBroadcastTxCommit) {
	...
	dr := depositReq{
		...
		Amount:    sdk.Coins{sdk.NewCoin(happ.StakeDenom, sdk.NewInt(amount))},
		BaseReq:   baseReq,
	}
