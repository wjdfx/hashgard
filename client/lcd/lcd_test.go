...
const (
	...
	name3 = "test3"
	memo  = "LCD test tx"
	pw    = happ.DefaultKeyPass
	altPw = "12345678901"
)

var fees = sdk.Coins{sdk.NewInt64Coin(happ.StakeDenom, 5)}
...
func TestCoinSend(t *testing.T) {
	...
	require.Equal(t, happ.StakeDenom, coins[0].Denom)
	...
	require.Equal(t, happ.StakeDenom, coins2[0].Denom)
	...
	acc = getAccount(t, port, addr)
	require.Equal(t, expectedBalance.Amount, acc.GetCoins().AmountOf(happ.StakeDenom))
	...
	require.Equal(t, expectedBalance.Amount.SubRaw(1), acc.GetCoins().AmountOf(happ.StakeDenom))
}
...
func TestPoolParamsQuery(t *testing.T) {
	...
	initialPool.BondedTokens = initialPool.BondedTokens.Add(sdk.NewInt(100))      // Delegate tx on HashgardAppGenState
	initialPool.NotBondedTokens = initialPool.NotBondedTokens.Add(sdk.NewInt(50)) // freeFermionsAcc = 50 on HashgardAppGenState
...
func TestBonding(t *testing.T) {
	...
	coins := acc.GetCoins()
	expectedBalance := initialBalance[0].Minus(fees[0])
	require.Equal(t, expectedBalance.Amount.SubRaw(60), coins.AmountOf(happ.StakeDenom))
	expectedBalance = coins[0]
	...
	expectedBalance = expectedBalance.Minus(fees[0])
	require.True(t,
		expectedBalance.Amount.LT(coins.AmountOf(happ.StakeDenom)) ||
			expectedBalance.Amount.Equal(coins.AmountOf(happ.StakeDenom)),
...
func TestSubmitProposal(t *testing.T) {
	...
	acc = getAccount(t, port, addr)
	expectedBalance := initialBalance[0].Minus(fees[0])
	require.Equal(t, expectedBalance.Amount.SubRaw(5), acc.GetCoins().AmountOf(happ.StakeDenom))
...
func TestDeposit(t *testing.T) {
	...
	coins := acc.GetCoins()
	expectedBalance := initialBalance[0].Minus(fees[0])
	require.Equal(t, expectedBalance.Amount.SubRaw(5), coins.AmountOf(happ.StakeDenom))
	expectedBalance = coins[0]
	...
	// query proposal
	proposal = getProposal(t, port, proposalID)
	require.True(t, proposal.GetTotalDeposit().IsEqual(sdk.Coins{sdk.NewInt64Coin(happ.StakeDenom, 10)}))

	// query deposit
	deposit := getDeposit(t, port, proposalID, addr)
	require.True(t, deposit.Amount.IsEqual(sdk.Coins{sdk.NewInt64Coin(happ.StakeDenom, 10)}))
}
...
func TestVote(t *testing.T) {
	...
	coins := acc.GetCoins()
	expectedBalance := initialBalance[0].Minus(fees[0])
	require.Equal(t, expectedBalance.Amount.SubRaw(10), coins.AmountOf(happ.StakeDenom))
	expectedBalance = coins[0]
	...
	coins = acc.GetCoins()
	expectedBalance = expectedBalance.Minus(fees[0])
	require.Equal(t, expectedBalance.Amount, coins.AmountOf(happ.StakeDenom))
	expectedBalance = coins[0]
...
func TestProposalsQuery(t *testing.T) {
	...
	halfMinDeposit := depositParam.MinDeposit.AmountOf(happ.StakeDenom).Int64() / 2
